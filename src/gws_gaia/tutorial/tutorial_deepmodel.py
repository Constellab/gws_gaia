...
from gws_gaia.tf import (Conv2D, MaxPooling2D, Flatten, Dropout, Dense,
                            PickleImporter, Preprocessor, AdhocExtractor,
                            Tensor, DeepModel, InputConverter,
                            DeepModelBuilder, DeepModelCompiler, DeepModelerTrainer,
                            DeepModelerPredictor)

@protocol_decorator("DeepMoldelTurorialProto",
                    human_name="Deep Moldel Proto",
                    short_description="Turorial: Deep moldel protocol", hide=True)
class DeepMoldelTurorialProto(Protocol):
    def configure_protocol(self, config_params: ConfigParams) -> None:
        settings = Settings.retrieve()
        test_dir = settings.get_variable("gws_gaia:testdata_dir")
        pkl_importer: ProcessSpec = self.add_process(PickleImporter, 'pkl_importer')
        ...
        deep_modeler_builder: ProcessSpec = self.add_process(DeepModelBuilder, 'deep_modeler_builder')
        deep_modeler_compiler: ProcessSpec = self.add_process(DeepModelCompiler, 'deep_modeler_compiler')
